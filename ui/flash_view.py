"""
ui/flash_view.py — Raw high-speed Gemma interactions.

Purpose:
  - This tab is completely disconnected from the Vault.
  - It does NOT use RAG or Tool schemas.
  - Pure, low-latency API passthrough to the local LLM.
"""
...
class Bubble(QFrame):
    def __init__(self, text: str, is_user: bool, parent=None):
        ...
        if is_user:
            ...
            lbl.setStyleSheet(
                "background:#1a1100;border:1px solid rgba(255,165,0,0.4);" # Orange tint for Flash
                "color:rgba(255,255,255,0.9);border-radius:14px;"
...
class ThinkingBubble(QFrame):
    def __init__(self, parent=None):
        ...
        lbl = QLabel("Flashing\u2026")
        lbl.setStyleSheet(
            "background:#0a0a0a;border:1px solid rgba(255,165,0,0.2);"
            "color:rgba(255,165,0,0.6);border-radius:14px;"
...
class FlashView(QWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        self._worker: AIWorker | None = None
        self._thinking_bubble: ThinkingBubble | None = None
        
        # Local history strictly for this Flash session.
        # It is intentionally completely isolated from the ConversationManager 
        # so flash questions don't pollute the user's permanent AI context.
        self._flash_history = []
    ...
    def _build_ui(self):
        ...
        title = QLabel("Flash")
        title.setStyleSheet(
            "color:#ffaa00;font-size:16px;font-weight:600;" # Orange accent
            "letter-spacing:-0.3px;background:transparent;")
        sub = QLabel(
            "Maximum-speed local intelligence. No vault access. Raw API speed.")
        sub.setStyleSheet(
        ...
        self._empty_lbl = QLabel(
            "⚡ Flash Mode Ready.\n"
            "This chat bypasses all RAG retrieval and tool schemas.\nIt talks directly to the raw model for instant latency.")
        self._empty_lbl.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self._empty_lbl.setStyleSheet(
            "color:rgba(255,165,0,0.4);font-size:14px;font-style:italic;"
        ...
        self._send_btn.setStyleSheet(
            "background-color: #d68100;" # Orange
            "color: #ffffff;"
            "border-radius: 6px;"
        )
    ...
    def _send(self):
        ...
        self._worker = AIWorker(text, self._flash_history, mode="flash")
    ...
    def _on_result(self, reply: str, new_history: list):
        ...
        self._flash_history = new_history
